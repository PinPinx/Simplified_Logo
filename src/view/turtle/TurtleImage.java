...
public class TurtleImage extends ImageView {
	...
	public TurtleImage(GraphicsContext gcon, int id) {
		this(gcon, id, DEFAULT_IMAGEPATH, DEFAULT_XPOS, DEFAULT_YPOS,
				DEFAULT_WIDTH, DEFAULT_HEIGHT);
	}

	public TurtleImage(GraphicsContext gcon, int id, String imagePath,
	...
	public void processUpdate(TurtleUpdate tu) {
		...
		if (!penUp) {
			myPen.drawLine(
					new Point2D(oldPos.getX() + myWidth / 2, oldPos.getY()
							+ myHeight / 2), new Point2D(newPos.getX()
							+ myWidth / 2, newPos.getY() + myHeight / 2));
		}
	...
	private void animatedMove(double toAngle, Point2D toLocation) {
		...
		SequentialTransition sequentialTransition = new SequentialTransition(
				this, rotateTransition, translateTransition);
	...
	private void initializeMenu() {
		...
		contextMenu = new ContextMenu(changeImage, toggle, penUpDown,
				lineWidth, lineStyle, penColor);
	...
	private Transition createTranslateTransition(Point2D destination) {
		...
		TranslateTransition tt = new TranslateTransition(
				Duration.millis(distance / mySpeed), this);
	...
	private void selectPenColor() {
		java.awt.Color awtColor = JColorChooser.showDialog(null,
				"Choose color to add to palette", null);
		Color fxColor = Color.rgb(awtColor.getRed(), awtColor.getGreen(),
				awtColor.getBlue());
	...
	/**
	 * Generates a toggle group for line styles (solid, dashed, dotted,
	 * dash-dot)
	 */
	...
	private Point2D mathCoordsToCanvasCoords(Point2D mathCoords) {
		...
		return new Point2D(mathCoords.getX()
				+ (gc.getCanvas().getWidth() / 2 - myWidth / 2),
				-mathCoords.getY()
						+ (gc.getCanvas().getHeight() / 2 - myHeight / 2));
	}

	private Point2D canvasCoordsToMathCoords(Point2D canvasCoords) {
		return new Point2D(canvasCoords.getX()
				- (gc.getCanvas().getWidth() / 2 - myWidth / 2),
				-canvasCoords.getY()
						+ (gc.getCanvas().getHeight() / 2 - myHeight / 2));
	}
