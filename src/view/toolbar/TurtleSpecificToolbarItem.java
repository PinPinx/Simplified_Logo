...
/**
 * Class for toolbar items whose action(s) affect specific turtle, or turtle
 * group.
 * 
 * @author lien
 *
...
public abstract class TurtleSpecificToolbarItem extends ToolbarItem {
	...
	/**
	 * Ask for user input through a dialog box. Parse user input and update the
	...
	private void getUserInput() {
		...
		if (userInput.equals("all")) {
			selectedTurtles.addAll(myToolbar.getTurtleWindow()
					.getAvailableTurtles());
		} else {
	...
	/**
	 * Gets an (unmodifiable) list of currently available turtles and arrange it
	 * in a String to be displayed in the prompt.
	 * 
	 * @return String listing currently available turltes.
	 */
	private String getAvailableTurtles() {
		List<Integer> turtleIDs = myToolbar.getTurtleWindow()
				.getAvailableTurtles();
